...
const styles = {
  ...
  text:'#2C3E50',
  dark:'#083E8C',
  card:{ background:'#fff', border:'1px solid #E6EEF9', borderRadius:12, padding:16, marginBottom:12 }
}
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      setDtes(await api('/dte'))
      setExps(await api('/expenses'))
    })()
  },[])
  ...
  return (
    <div style={{ fontFamily:'Montserrat, system-ui, sans-serif', background:styles.bg, color:styles.text, minHeight:'100vh', padding:16, maxWidth:1100, margin:'0 auto' }}>
      <header style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        <div>
          <h1 style={{ margin:0, color:styles.dark }}>SEES-DTE</h1>
          <small>Dashboard y Programación de DTE</small>
        </div>
      </header>
      ...
      <section style={styles.card}>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          ...
          <input placeholder="Descripción" value={gasto.descripcion} onChange={e=>setGasto({...gasto, descripcion:e.target.value})} />
          <input placeholder="Monto" type="number" value={gasto.monto} onChange={e=>setGasto({...gasto, monto:e.target.value})} />
          <button onClick={async()=>{
            const created = await api('/expenses', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(gasto) })
            setExps([created, ...exps])
          }} style={{ background:styles.c, color:'#fff', border:'none', padding:'8px 12px', borderRadius:8 }}>Agregar</button>
        </div>
      </section>
      ...
      <section style={styles.card}>
        ...
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {exps.map(e=>(<tr key={e.id}><td>{e.fecha}</td><td>{e.descripcion}</td><td>${e.monto.toFixed(2)}</td></tr>))}
            </tbody>
          </table>
        </div>
      </section>
